...
import { generateUseCaseInterface, generateUseCase, generateRequestDto, generateResponseDto } from "./templates/useCaseLayer";
...
async function generateInterfaceAdapterLayer() {
    ...
    const repositryContent = generatePrismaRepository(entityName);
    writeFile(
        path.join(
            basePath,
            "repositories",
            `prisma${lowercaseFirst(entityName)}Repositry.ts`
        ),
        repositryContent
    );
}
